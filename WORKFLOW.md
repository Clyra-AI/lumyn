...
the Factory `autoship-supervisor` skill from the Lumyn repo root and bind it to
exactly one task ID.
...
delivery-slice labels instead of item-level evidence.
...
- a product task omits Factory `prd-to-plan` / `execution-compiler` fields for Factory compatibility, explicit scope exclusions, alignment gate refs, plan-drift refs, acceptance-ledger refs, acceptance item IDs, runtime pins, slice rationale, changelog/versioning intent, contract impact, ADR posture, TDD-first evidence, cost/perf impact, failure hypotheses, semantic invariants, canonical worker chains, or lifecycle gates
- a runner-ready product task omits `worker_type`, `factoryd_runtime`, `validation_commands`, `max_iterations`, `evidence_required`, or `stop_conditions`
