...
M1 splits the full milestone contract from its attended implementation phase.
The parent retains private holdout, independent review, remote CI execution or
monitoring, shipping, and post-merge gates. The child packet permits only
public/synthetic fixture, test, validator, task-readable docs, and exact
CI/toolchain configuration writes plus local validation. Planning truth stays
owned by `plan-verify-repair`. Its deterministic route is agent-free; a
separate fake adapter exercises the clean-session, normalized-output,
cancellation, and
no-fallback boundary without credentials, network, or external writes. The
agent scenarios use real local imports and calls across consumer-side fixture
files; their site-ID chains and primary behavior checks must prove that composition,
not merely list disconnected files. Fixture execution receives no ambient host
environment and runs stripped TypeScript inside a code-generation-disabled VM
whose linker accepts only relative modules inside the fixture root and whose
...
conditional on a fresh read-only verification view and process running the
exactly authorized pinned offline `npm test --silent` command with provider
reporting disabled. The trusted developer harness creates a real deterministic
Git baseline and candidate commit, verifies the candidate commit object and
clean tree, and executes the exact generated bytes from that candidate head; an
expected-failing baseline runs in a separate process and cannot be replaced by
the checked-in expected candidate. Only a typed target-contract mismatch counts
as red-before evidence; missing, parse, link, and top-level failures block.
Separate command and output digests, the package and verification-runner
bytes, complete fixture, exact
candidate, toolchain, and verified cleanup of the view and isolated npm roots
are bound into the canonical verification and export artifacts. It carries no
item-level acceptance closure. Candidate writes and trusted validation
...
separately. Node `vm`, cleared environment, and read-only modes are not a
security boundary. This developer harness cannot claim `repo_verified` or
authorize product repository commands; that requires the later pinned qualified
host-isolation backend with enforced mounts, egress, quotas, and cleanup.
