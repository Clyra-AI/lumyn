...
| M1 | fixture and contract | benchmark prerequisites | provenance, holdout | agent-free deterministic route; test-only fake adapter; live adapters deferred to M6/M10 |
...
**Implementation status:** The attended local phase now includes six frozen
public/synthetic TypeScript scenarios, seven blocked classes, the matched
generic-agent comparison protocol, a source-safe holdout provisioning
manifest, executable pinned offline behavior checks, three coordinated
multi-file agent-route fixtures, and an agent-free deterministic
event-to-local-PR-bundle skeleton. This is nonterminal prerequisite evidence
only. The skeleton validates canonical M2 artifacts and the production
authorization boundary, then earns only a synthetic-fixture `static_verified`
label. The trusted developer harness creates a deterministic Git baseline,
copies the exact generated candidate into that repository, creates and verifies
a distinct candidate commit and clean tree, then runs separate expected-failing
baseline and passing exact-head candidate
`npm test --silent` processes with provider reporting disabled, and binds the
package, runner, candidate, fixture, command, output, toolchain, and verified
cleanup evidence into the verification and export chain. A checked-in expected
candidate cannot substitute for the generated bytes, and only a typed
target-contract mismatch—not a missing, parse, link, or top-level failure—can
satisfy red-before proof. Complete normalized artifact digests cover every
semantic field; explicit self-digests and the authorization's later plan
binding are the only cycle breakers, and cross-artifact bindings are checked
separately. Node `vm`, cleared environment, and file modes are not qualified
host isolation, so `repo_verified` and product repository command execution
remain blocked until a later milestone provides the pinned qualified backend.
...
- Create at least three agent-assisted scenarios involving wrappers, adapters,
  ...
  decisions. Each visible agent-route scenario must coordinate at least two
  consumer-side fixture files through executable local imports and calls, bind
  graph `via` chains to concrete site IDs across that boundary, expose distinct
  context-dependent edits, and omit a provider-authored deterministic recipe.
...
- Using public or synthetic fixtures, prove the thin sequence `event ->
  ...
  the canonical M2 schemas and production authorization boundary. The
  deterministic route invokes no agent, creates real deterministic Git
  baseline and candidate commits, copies the exact generated candidate into a fresh
  read-only view at the verified candidate head, executes an expected-failing
  baseline separately from the passing candidate using the authorized pinned
  offline `npm test --silent` command with provider reporting disabled, binds both
  command and output digests plus package, runner, candidate, complete fixture,
  toolchain, and verified cleanup evidence into canonical verification, and
  stops at a manual local PR bundle.
  This trusted developer harness earns only `static_verified`; it is not a
  qualified host-isolation backend and cannot authorize product repository
  command execution or earn `repo_verified`.
  Exercise the common Agent Runner contract separately with a deterministic
  fake. This walking skeleton proves contract composition only; it cannot close
  adapter qualification, automated GitHub delivery, real-repository, or
  commercial acceptance.
- Keep fixture and product-command execution offline and credential-free. It
  must not launch Codex, Claude Code, Cursor, or any live model, consume Agent Runner/model
  credentials, or receive product network grants. Lifecycle-owned remote CI
  may perform the pinned action, toolchain, and dependency bootstrap required
  by the declared workflows. Freeze the common harness and matched-baseline
  protocol here; M6 owns qualified adapter conformance and live canaries, and
  M10 owns the matched live generic-agent comparison.
- Pin every visible fixture to Node 22.16.0 and npm 11.4.1, keep it
  dependency-free, execute TypeScript through Node's pinned native
  `--experimental-strip-types` path in a no-host-global, local-import-only VM,
  clear the ambient environment, isolate npm configuration/cache/home/temp,
  create assertions and mocks inside the same VM realm without injecting host
  objects or functions, reject direct and computed-constructor recovery of
  process, dynamic-code, and network capabilities, fail closed on an
  ambient version mismatch, reject substitution of checked-in expected bytes
  for the generated candidate, reject missing/parse/link/top-level failures as
  red evidence, and require its offline `npm test --silent` to prove typed
  red-before and green-candidate target behavior for every expected edited file
  in separate processes.

#### Implementation repo areas
...
Planning prerequisites remain owned by `plan-verify-repair`, not
`task-executor`: `AGENTS.md`, `WORKFLOW.md`, `docs/product/prd.md`,
`docs/product/plan.md`, and
`.factory/artifacts/prd-to-plan/lumyn-migration-mvp/`. They must be
regenerated and validated before candidate validation, then bound into the
same release candidate.

The executable local phase is compiled separately at
`.factory/artifacts/prd-to-plan/lumyn-migration-mvp/m1-attended-implementation-task-packet.json`.
Its Factory schema snapshot is pinned under `.factory/contracts/factory/` so
repo validation cannot accept a packet the bound Factory schema rejects.
...
- Walking-skeleton contract, event replay, manual-bundle boundary, and separate
  fake-adapter conformance tests.
...
- Negative tests proving M1 cannot launch a live Agent Runner/model or accept
  Agent Runner/model credentials or product-command, Agent Runner, or model
  network grants.
...
- The public/synthetic walking skeleton composes the schema-valid provider
  event, installation, impact, plan, candidate, verification, and manual PR
  bundle chain without an external write.
- A real deterministic Git baseline and candidate head produce a typed
  expected-failing baseline and an exact-head passing candidate under the
  pinned `npm test --silent` command, complete normalized artifact bindings,
  verified cleanup, and no label stronger than `static_verified`.
- The deterministic walking-skeleton route invokes no agent; a separate fake
  adapter passes only common Agent Runner contract conformance and cannot
  generate, verify, or deliver the candidate.
- Independent review, private-holdout provisioning, required CI and CodeQL,
